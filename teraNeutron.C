    ...
    for(int i=0;i<nentries;i++)
    {
        ...
        {//first particle
            ...
            for(int k=j+1;k<nMultiplicityTree;k++){
                //second particle
                if(std::find(paired.begin(),paired.end(),ientry) !=paired.end())    continue;
                TLorentzVector p2,r2;
                p2.SetPxPyPzE(px[k],py[k],pz[k],energy[k]);
                r2.SetPxPyPzE(x[k],y[k],z[k],t[k]);

                if(!coalescence(p1,p2,r1,r2)) continue;
                for(int l=k+1;l<nMultiplicityTree;l++){
                    //third particle
                    if(std::find(paired.begin(),paired.end(),ientry) !=paired.end())    continue;

                    TLorentzVector p2,r2;
                    p2.SetPxPyPzE(px[l],py[l],pz[l],energy[l]);
                    r2.SetPxPyPzE(x[l],y[l],z[l],t[l]);
                    if(!coalescence(p1,p3,r1,r3) || !coalescence(p2,p3,r2,r3))  continue;
                    for(int m=l+1;m<nMultiplicityTree;m++){//forth particle
                        
                        if(std::find(paired.begin(),paired.end(),ientry) !=paired.end())    continue;
                        TLorentzVector p4,r4;
                        p4.SetPxPyPzE(px[m],py[m],pz[m],energy[m]);
                        r4.SetPxPyPzE(x[m],y[m],z[m],t[m]);
                        if(!colescence(p1,p4,r1,r4)) continue;
                        if(!coalescence(p2,p4,r2,r4)) continue;
                        if(!coalescence(p3,p4,r3,r4)) continue;
                        TLorentzVector four;
                        four=p1+p2+p3+p4;
                        float pt = four.Perp();
